...
# 设置文件路径（确保路径正确）
file_path <- "D:/Desktop/9-22/NKTCL - 副本/3-lassoPFIout0922.csv"
...
# 根据系数正负划分保护基因和危险基因
protective_genes <- names(coefficients[coefficients > 0])  # 保护基因
risk_genes <- names(coefficients[coefficients < 0])        # 危险基因

# 输出保护基因和危险基因
cat("保护基因:\n")
print(protective_genes)
...
# 提取危险基因和保护基因对应的行
gene_rows <- data[c(protective_genes, risk_genes), ]

# 合并生存信息和基因信息
final_output <- rbind(survival_info, gene_rows)

# 保存为 CSV 文件
write.csv(final_output, "D:/Desktop/9-22/NKTCL/4-step_pfi.csv", row.names = TRUE)

cat("生存信息和基因信息已保存至 '4-step_pfi.csv'.\n")
